...
logger = get_logger("GreenhousePlatform")
...
class Greenhouse(BasePlatform):
    ...
    async def search_jobs(self, keywords: List[str], location: str, pages: int = 1) -> List[Dict[str, Any]]:
        query = " ".join(keywords)
        logger.info(f"Searching Greenhouse for: {query} in {location}")
        ...
        for page in range(pages):
            extracted = self.extract_job({
                "title": f"Software Engineer (Simulated Greenhouse {page})",
                "company": "Mock Greenhouse Company",
                "location": location,
                "job_url": f"https://greenhouse.com/jobs/123",
