...
logger = get_logger("WellfoundPlatform")

class Wellfound(BasePlatform):
    def __init__(self):
        super().__init__()
        self.session = None
    ...
    async def search_jobs(self, keywords: List[str], location: str, pages: int = 1) -> List[Dict[str, Any]]:
        query = " ".join(keywords)
        logger.info(f"Searching Wellfound for: {query} in {location}")
        ...
        for page in range(pages):
            extracted = self.extract_job({
                "title": f"Software Engineer (Simulated Wellfound {page})",
                "company": "Mock Wellfound Company",
                "location": location,
                "job_url": f"https://wellfound.com/jobs/123",
